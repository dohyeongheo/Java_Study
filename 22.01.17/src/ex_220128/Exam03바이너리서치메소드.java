...
public class Exam03바이너리서치메소드 {
	...
	private static void binarysearch1(int num1) {
		...
		while (low <= high) {
			mid = (low + high) / 2;
			if (array[mid] == num1) {
				System.out.println(num1 + "은 " + mid + "번째 인덱스에 있습니다.");
				ischeck = false;
				break;
		...
		if (ischeck)
			System.out.println(num1 + "은 인덱스 내에 존재하지 않습니다.");
	}

}

//		if (ischeck) System.out.println(searchNumber + "은 인덱스 내에 존재하지 않습니다.");
